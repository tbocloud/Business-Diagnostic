...
//  Saves submission to MySQL, then calls OpenAI
// ─────────────────────────────────────────────
...
// ── Save submission to MySQL FIRST ────────────
$submissionId = null;
$pdo          = null;
...
if ($scores && $answers) {
    ...
    try {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO submissions
              (company_name, industry, business_age, business_type, team_size,
               ...
               score_ai_readiness, score_average, raw_answers)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        );
        ...
        error_log('MySQL insert failed: ' . $e->getMessage());
    }
}
...
if (!defined('OPENAI_API_KEY') || str_starts_with(OPENAI_API_KEY, 'sk-your')) {
    ...
    echo json_encode(['error' => 'Add your OPENAI_API_KEY in config.php', 'submissionId' => $submissionId]);
    exit;
}
...
if ($httpStatus !== 200) {
    ...
    echo json_encode(['error' => 'OpenAI error ' . $httpStatus, 'submissionId' => $submissionId]);
    exit;
}
...
if (!$raw) {
    http_response_code(502);
    echo json_encode(['error' => 'Empty OpenAI response', 'submissionId' => $submissionId]);
    exit;
}
...
if (!$gptOutput) {
    http_response_code(502);
    echo json_encode(['error' => 'Invalid JSON from OpenAI', 'submissionId' => $submissionId]);
    exit;
}

// ── Update row with GPT output ────────────────
if ($submissionId && $pdo) {
    try {
        $stmt = $pdo->prepare(
            'UPDATE submissions SET overall_rating = ?, gpt_output = ? WHERE id = ?'
        );
        $stmt->execute([
        ...
        error_log('MySQL update failed (non-fatal): ' . $e->getMessage());
    }
}
